function [y_nonoise,img,truevalues,pixelnoisevar]=fw(SNR,vobj,vkminimalset,Nv,NT,Na,Nb,rule,all_tilde_b,ixminimalset,iyminimalset,deltachi)
%function [y_nonoise,img,truevalues,pixelnoisevar]=fw(SNR,vobj,vkminimalset,Nv,NT,Na,Nb,rule,all_tilde_b,ixminimalset,iyminimalset,deltachi)
...
for ii=1:Nv
  ...
  L=setL_nord(Rabc, vobj{eta}.clnp.il, vobj{eta}.clnp.in, vkminimalset, vobj{eta}.Htable, vobj{eta}.map_unique2lp, all_tilde_b{eta}.tilde_b, vobj{eta}.Ftable, vobj{eta}.BasisFunctionType);
  if ~isreal(L)
    error('fw: ii %d L is not real\n',ii);
  end
  rankL=rank(L);
  if (rankL < Nc)
    fprintf(2,'L is not full rank! ii %d rank(L) %d Nc %d size(vkminimalset,1) %d.\n',ii,rankL,Nc,size(vkminimalset,1))
  end
  ...
  img{ii}=(1/(deltachi(1)*deltachi(2)))*fftshift(ifft2(ImgAs2DComplexImage));
  if ~isfinite(img{ii})
    error('fw: ii %d isfinite\n',ii);
  end
  if ~isreal(img{ii})
    fprintf(1,'fw: img{%d} fails isreal\n',ii);
    if all(abs(imag(img{ii}))<abs(img{ii})*1.0e-10)
      img{ii}=real(img{ii});
      fprintf(1,'fw: img{%d} fails isreal is fixed\n',ii);
    else
      fprintf(1,'fw: img{%d} fails isreal could not be fixed\n',ii);
      error('fw: exiting');
    end
  end

  mean_all_pixels=mean_all_pixels+sum(sum(img{ii}));
end
