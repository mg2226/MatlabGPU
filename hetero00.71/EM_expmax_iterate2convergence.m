...
%%%  fprintf(1,'EM_expmax_iterate2convergence: Iteration %d\n',iter);
  ...
  %set loglike_history(iter) below.
...
%%%    fprintf(1,'EM_expmax_iterate2convergence: iter %d compute pixelnoisevarNEW (pixelnoisevar %g)\n',iter,pixelnoisevar);
    ...
    pixelnoisevarNEW=set_Q_noise_solve(EM_iter.rule,vk,y,vobj,p_theta_eta,EM_iter.Na, iter);
...
%%%  disp_v(EM_iter.verbosity,1,qNEW');
  ...
  absoluteloglike=iter>1 && ...
    (EM_iter.loglikeftol<0.0 || abs(loglike_history(iter)-loglike_history(iter-1))<=EM_iter.loglikeftol);
  relativeloglike=iter>1 && ...
    (EM_iter.loglikertol<0.0 || abs(loglike_history(iter)-loglike_history(iter-1))/(0.5*(abs(loglike_history(iter))+abs(loglike_history(iter-1))))<=loglikertol);
